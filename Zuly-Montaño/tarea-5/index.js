class GamePuzzle {
    ...
    init() {
      for (let a = 0; a < this.rows; a++) {
        for (let b = 0; b < this.cols; b++) {
          let blockId = b + a * this.cols;
          if (blockId + 1 >= this.count) break;
          let block = this.blocks[blockId];
          this.positionBlock(blockId, b, a);
    ...
    positionBlock(blockId, b, a) {
      let block = this.blocks[blockId];
      block.style.left = b * block.clientWidth + 'px';
      block.style.top = a * block.clientWidth + 'px';
    }
  }
  var puzzle = new GamePuzzle();